...
namespace Iot.Device.Bno08X
{
    ...
    struct GetFeatureReq_t
    {
        uint reportId;
        uint featureReportId;
    };
    ...
    struct CommandReq_t
    {
        ...
        uint seq;
        uint command;
        uint p[COMMAND_PARAMS];
    };

    struct CommandResp_t
    {
        uint reportId;
        uint seq;
        uint command;
    ...
    typedef uint (sh2_OpStart_t) (sh2_t* pSh2);
    typedef void (sh2_OpRx_t) (sh2_t* pSh2, const int* payload, uint len);
...
;
    ...
    public class sh2
    {
        ...
        static uint GUID_EXECUTABLE = 1;
        const uint GUID_SENSORHUB = 2;

        // executable/device channel responses
        const uint EXECUTABLE_DEVICE_CMD_RESET = 1;
        const uint EXECUTABLE_DEVICE_CMD_ON = 2;
        const uint EXECUTABLE_DEVICE_CMD_SLEEP = 3;

        // executable/device channel responses
        const uint EXECUTABLE_DEVICE_RESP_RESET_COMPLETE = 1;

        // Tags for sensorhub app advertisements.
        const uint TAG_SH2_VERSION = 0x80;
        const uint TAG_SH2_REPORT_LENGTHS = 0x81;

        // Max length of sensorhub version string.
        const uint MAX_VER_LEN = 16;

        // Max number of report ids supported
        const uint SH2_MAX_REPORT_IDS = 64;

#if defined(_MSC_VER)
const uint  PACKED_STRUCT struct
#pragma pack(push, 1)
#elif defined(__GNUC__)
const uint  PACKED_STRUCT struct __attribute__((packed))
#else
#define PACKED_STRUCT __packed struct
#endif

        const uint ADVERT_TIMEOUT_US = 200000;

        // Command and Subcommand values
        const uint SH2_CMD_ERRORS = 1;
        const uint SH2_CMD_COUNTS = 2;
        const uint SH2_COUNTS_GET_COUNTS = 0;
        const uint SH2_COUNTS_CLEAR_COUNTS = 1;
        const uint SH2_CMD_TARE = 3;
        const uint SH2_TARE_TARE_NOW = 0;
        const uint SH2_TARE_PERSIST_TARE = 1;
        const uint SH2_TARE_SET_REORIENTATION = 2;
        const uint SH2_CMD_INITIALIZE = 4;
        const uint SH2_INIT_SYSTEM = 1;
        const uint SH2_INIT_UNSOLICITED = 0x80;
        // const uint  SH2_CMD_FRS             =       5; /* Depreciated */
        const uint SH2_CMD_DCD = 6;
        const uint SH2_CMD_ME_CAL = 7;
        const uint SH2_CMD_DCD_SAVE = 9;
        const uint SH2_CMD_GET_OSC_TYPE = 0x0A;
        const uint SH2_CMD_CLEAR_DCD_AND_RESET = 0x0B;
        const uint SH2_CMD_CAL = 0x0C;
        const uint SH2_CAL_START = 0;
        const uint SH2_CAL_FINISH = 1;
        const uint SH2_CMD_BOOTLOADER = 0x0D;     /* SH-2 Reference Manual 6.4.12 */
        const uint SH2_BL_MODE_REQ = 0;
        const uint SH2_BL_STATUS_REQ = 1;
        const uint SH2_CMD_INTERACTIVE_ZRO = 0x0E;     /* SH-2 Reference Manual 6.4.13 */

        // SENSORHUB_COMMAND_REQ
        const uint SENSORHUB_COMMAND_REQ = 0xF2;
        const uint COMMAND_PARAMS = 9;


        // SENSORHUB_COMMAND_RESP
        const uint SENSORHUB_COMMAND_RESP = 0xF1;
        const uint RESPONSE_VALUES = 11;


        // SENSORHUB_PROD_ID_REQ
        const uint SENSORHUB_PROD_ID_REQ = 0xF9;
        struct ProdIdReq_t
        {
            uint reportId;
            uint reserved;
        ...
        const uint SENSORHUB_PROD_ID_RESP = 0xF8;
        struct ProdIdResp_t
        {
            uint reportId;
            uint resetCause;
        ...
        const uint FEAT_CHANGE_SENSITIVITY_RELATIVE = 1;
        const uint FEAT_CHANGE_SENSITIVITY_ABSOLUTE = 0;
        const uint FEAT_CHANGE_SENSITIVITY_ENABLED = 2;
        const uint FEAT_CHANGE_SENSITIVITY_DISABLED = 0;
        const uint FEAT_WAKE_ENABLED = 4;
        const uint FEAT_WAKE_DISABLED = 0;
        const uint FEAT_ALWAYS_ON_ENABLED = 8;
        const uint FEAT_ALWAYS_ON_DISABLED = 0;

        // GET_FEATURE_REQ
        const uint SENSORHUB_GET_FEATURE_REQ = 0xFE;


        // SENSORHUB_GET_FEATURE_RESP
        const uint SENSORHUB_GET_FEATURE_RESP = 0xFC;
        struct GetFeatureResp_t
        {
            uint reportId;
            uint featureReportId;      // sensor id
            uint flags;                // FEAT_... values
            uint changeSensitivity;
            uint reportInterval_uS;
            uint batchInterval_uS;
            uint sensorSpecific;
        }
        ;
        ...
        uint frsData[MAX_FRS_WORDS];
        uint frsDataLen;

        // Stats
        uint execBadPayload;
        uint emptyPayloads;
        uint unknownReportIds;

    };

    const uint SENSORHUB_BASE_TIMESTAMP_REF (0xFB)
    ...
    BaseTimestampRef_t;
        ...
        const uint SENSORHUB_TIMESTAMP_REBASE   (0xFA)
        typedef PACKED_STRUCT
    {
            uint reportId;
        int32_t timebase;
    ...
    TimestampRebase_t;
        ...
        const uint SENSORHUB_FORCE_SENSOR_FLUSH (0xF0)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint sensorId;
    ...
    ForceFlushReq_t;
        ...
        const uint SENSORHUB_FLUSH_COMPLETED    (0xEF)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint sensorId;
    ...
    static uint opStart(sh2_t* pSh2, const sh2_Op_t* pOp)
    {
        ...
        uint rc = pOp->start(pSh2);  // Call start method
        if (rc != SH2_OK)
        {
    ...
    static void opRx(sh2_t* pSh2, const int* payload, uint len)
    {
        if ((pSh2->pOp != 0) &&                      // An operation is in progress
            (pSh2->pOp->rx != 0))
        {                  // and it has an rx method
            pSh2->pOp->rx(pSh2, payload, len);  // Call receive method
        }
    }

    static void sensorhubAdvertHdlr(void* cookie, uint tag, uint len, int* value)
    {
    ...
    static void sensorhubControlHdlr(void* cookie, int* payload, uint len, uint timestamp)
    {
        sh2_t* pSh2 = (sh2_t*)cookie;
    ...
    static uint opCompleted(sh2_t* pSh2, uint status)
    {
        // Record status
    ...
    static uint opProcess(sh2_t* pSh2, const sh2_Op_t* pOp)
    {
        uint status = SH2_OK;
        uint start_us = 0;
    ...
    static uint getReportLen(sh2_t* pSh2, uint reportId)
    {
        for (int n = 0; n < SH2_MAX_REPORT_IDS; n++)
        {
            if (pSh2->report[n].id == reportId)
    ...
    static uint touSTimestamp(uint hostInt, int32_t referenceDelta, uint delay)
    {
        static uint lastHostInt = 0;
        static uint rollovers = 0;
        uint timestamp;
        ...
        timestamp = ((uint)rollovers << 32);
        timestamp += hostInt + (referenceDelta + delay) * 100;

        return timestamp;
    }

    static void sensorhubInputHdlr(sh2_t* pSh2, int* payload, uint len, uint timestamp)
    {
        ...
        while (cursor < len)
        {
            ...
            // Determine report length
            uint reportLen = getReportLen(pSh2, reportId);
            if (reportLen == 0)
            {
                // An unrecognized report id
                pSh2->unknownReportIds++;
            ...
            else
            {
                ...
                {
                    int* pReport = payload + cursor;
                    uint delay = ((pReport[2] & 0xFC) << 6) + pReport[3];
    ...
    static void sensorhubInputNormalHdlr(void* cookie, int* payload, uint len, uint timestamp)
    {
        sh2_t* pSh2 = (sh2_t*)cookie;
    ...
    static void sensorhubInputWakeHdlr(void* cookie, int* payload, uint len, uint timestamp)
    {
        sh2_t* pSh2 = (sh2_t*)cookie;
    ...
    static void sensorhubInputGyroRvHdlr(void* cookie, int* payload, uint len, uint timestamp)
    {
        sh2_t* pSh2 = (sh2_t*)cookie;
        sh2_SensorEvent_t event;
        uint cursor = 0;

        uint reportId = SH2_GYRO_INTEGRATED_RV;
        uint reportLen = getReportLen(pSh2, reportId);
    ...
    static void executableAdvertHdlr(void* cookie, uint tag, uint len, int* value)
    {
        // Ignore.  No known TLV tags for this app.
    }

    static void executableDeviceHdlr(void* cookie, int* payload, uint len, uint timestamp)
    {
        sh2_t* pSh2 = (sh2_t*)cookie;
    ...
    static uint sendExecutable(sh2_t* pSh2, uint cmd)
    {
        return shtp_send(pSh2->pShtp, pSh2->executableChan, &cmd, 1);
    }

    static uint sendCtrl(sh2_t* pSh2, const int* data, uint len)
    {
    ...
    static uint getProdIdStart(sh2_t* pSh2)
    {
        uint rc = SH2_OK;
        ProdIdReq_t req;
        ...
        rc = sendCtrl(pSh2, (int*)&req, sizeof(req));

        return rc;
    }

    static void getProdIdRx(sh2_t* pSh2, const int* payload, uint len)
    {
    ...
    static uint getSensorConfigStart(sh2_t* pSh2)
    {
        uint rc = SH2_OK;
        GetFeatureReq_t req;
    ...
    static void getSensorConfigRx(sh2_t* pSh2, const int* payload, uint len)
    {
    ...
    const uint SENSORHUB_SET_FEATURE_CMD    (0xFD)
        typedef PACKED_STRUCT
    {
            uint reportId;             // 0xFD
            uint featureReportId;      // sensor id
    ...
    SetFeatureReport_t;
        ...
        static uint setSensorConfigStart(sh2_t* pSh2)
    {
    ...
    const uint SENSORHUB_FRS_WRITE_REQ      (0xF7)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint reserved;
    ...
    FrsWriteReq_t;
        ...
        const uint SENSORHUB_FRS_WRITE_DATA_REQ (0xF6)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint reserved;
    ...
    FrsWriteDataReq_t;
        ...
        const uint FRS_WRITE_STATUS_RECEIVED (0)
        const uint FRS_WRITE_STATUS_UNRECOGNIZED_FRS_TYPE (1)
        const uint FRS_WRITE_STATUS_BUSY (2)
        const uint FRS_WRITE_STATUS_WRITE_COMPLETED (3)
        const uint FRS_WRITE_STATUS_READY (4)
        const uint FRS_WRITE_STATUS_FAILED (5)
        const uint FRS_WRITE_STATUS_NOT_READY (6) // data received when not in write mode
        const uint FRS_WRITE_STATUS_INVALID_LENGTH (7)
        const uint FRS_WRITE_STATUS_RECORD_VALID (8)
        const uint FRS_WRITE_STATUS_INVALID_RECORD (9)
        const uint FRS_WRITE_STATUS_DEVICE_ERROR (10)
        const uint FRS_WRITE_STATUS_READ_ONLY (11)

        // SENSORHUB_FRS_WRITE_RESP
        const uint SENSORHUB_FRS_WRITE_RESP     (0xF5)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint status;
    ...
    FrsWriteResp_t;
        ...
        // RESP_FRS_READ_REQ
        const uint SENSORHUB_FRS_READ_REQ       (0xF4)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint reserved;
    ...
    FrsReadReq_t;
        ...
        // Status values
        const uint FRS_READ_STATUS_NO_ERROR                        0
        const uint FRS_READ_STATUS_UNRECOGNIZED_FRS_TYPE           1
        const uint FRS_READ_STATUS_BUSY                            2
        const uint FRS_READ_STATUS_READ_RECORD_COMPLETED           3
        const uint FRS_READ_STATUS_OFFSET_OUT_OF_RANGE             4
        const uint FRS_READ_STATUS_RECORD_EMPTY                    5
        const uint FRS_READ_STATUS_READ_BLOCK_COMPLETED            6
        const uint FRS_READ_STATUS_READ_BLOCK_AND_RECORD_COMPLETED 7
        const uint FRS_READ_STATUS_DEVICE_ERROR                    8

        // SENSORHUB_FRS_READ_RESP
        const uint SENSORHUB_FRS_READ_RESP      (0xF3)
        typedef PACKED_STRUCT
    {
            uint reportId;
            uint len_status;  // See FRS_READ... macros above
            uint wordOffset;
            uint data0;
            uint data1;
            uint frsType;
    ...
    FrsReadResp_t;
        ...
        static uint getFrsStart(sh2_t* pSh2)
    {
        uint rc = SH2_OK;
        FrsReadReq_t req;
    ...
    static void getFrsRx(sh2_t* pSh2, const int* payload, uint len)
    {
...
static void stuffMetadata(sh2_SensorMetadata_t* pData, uint* frsData)
{
    ...
    else if (pData->revision == 2)
    {
        ...
        memcpy(pData->sensorSpecific, (int*)&frsData[9], pData->sensorSpecificLen);
    ...
    else if (pData->revision == 4)
    {
        ...
        memcpy(pData->sensorSpecific, (int*)&frsData[10], pData->sensorSpecificLen);
    ...
    // Copy vendor id
    memcpy(pData->vendorId, (int*)&frsData[vendorIdOffset],
           pData->vendorIdLen);
}
...
static uint setFrsStart(sh2_t* pSh2)
{
    uint rc = SH2_OK;
    FrsWriteReq_t req;
...
static void setFrsRx(sh2_t* pSh2, const int* payload, uint len)
{
    ...
    {
        uint offset = pSh2->opData.setFrs.offset;
...
static uint sendCmd(sh2_t* pSh2, uint cmd, uint p[COMMAND_PARAMS])
{
    uint rc = SH2_OK;
    CommandReq_t req;
...
static uint sendCmd0(sh2_t* pSh2, uint cmd)
{
    uint p[COMMAND_PARAMS];
...
static uint sendCmd1(sh2_t* pSh2, uint cmd, uint p0)
{
    uint p[COMMAND_PARAMS];
...
static uint sendCmd2(sh2_t* pSh2, uint cmd, uint p0, uint p1)
{
    uint p[COMMAND_PARAMS];
...
static uint getErrorsStart(sh2_t* pSh2)
{
    // Initialize state
...
static void getErrorsRx(sh2_t* pSh2, const int* payload, uint len)
{
    ...
    else
    {
        ...
        unsigned uint index = pSh2->opData.getErrors.errsRead;
...
static uint getCountsStart(sh2_t* pSh2)
{
    return sendCmd2(pSh2, SH2_CMD_COUNTS, SH2_COUNTS_GET_COUNTS, pSh2->opData.getCounts.sensorId);
}

static void getCountsRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint sendCmdStart(sh2_t* pSh2)
{
    uint status = sendCmd(pSh2, pSh2->opData.sendCmd.req.command,
...
static uint reinitStart(sh2_t* pSh2)
{
    uint status = sendCmd1(pSh2, SH2_CMD_INITIALIZE, SH2_INIT_SYSTEM);

    return status;
}

static void reinitRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint saveDcdNowStart(sh2_t* pSh2)
{
    uint status = sendCmd0(pSh2, SH2_CMD_DCD);

    return status;
}

static void saveDcdNowRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint getOscTypeStart(sh2_t* pSh2)
{
    return sendCmd0(pSh2, SH2_CMD_GET_OSC_TYPE);
}

static void getOscTypeRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
    sh2_OscType_t* pOscType;
...
static uint setCalConfigStart(sh2_t* pSh2)
{
    uint p[COMMAND_PARAMS];
...
static void setCalConfigRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint getCalConfigStart(sh2_t* pSh2)
{
    uint p[COMMAND_PARAMS];
...
static void getCalConfigRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint forceFlushStart(sh2_t* pSh2)
{
    ForceFlushReq_t req;
    ...
    return sendCtrl(pSh2, (int*)&req, sizeof(req));
}

static void forceFlushRx(sh2_t* pSh2, const int* payload, uint len)
{
...
static uint startCalStart(sh2_t* pSh2)
{
    uint p[COMMAND_PARAMS];
...
static void startCalRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
...
static uint finishCalStart(sh2_t* pSh2)
{
    return sendCmd1(pSh2, SH2_CMD_CAL, SH2_CAL_FINISH);
}

static void finishCalRx(sh2_t* pSh2, const int* payload, uint len)
{
    CommandResp_t* resp = (CommandResp_t*)payload;
             ...
             sh2_EventCallback_t* eventCallback, void* eventCookie)
{
    ...
    uint start_us = pSh2->pHal->getTimeUs(pSh2->pHal);
    uint now_us = start_us;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setSensorCallback(sh2_SensorCallback_t* callback, void* cookie)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getProdIds(sh2_ProductIds_t* prodIds)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getSensorConfig(sh2_SensorId_t sensorId, sh2_SensorConfig_t* pConfig)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setSensorConfig(sh2_SensorId_t sensorId, const sh2_SensorConfig_t* pConfig)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getMetadata(sh2_SensorId_t sensorId, sh2_SensorMetadata_t* pData)
{
    sh2_t* pSh2 = &_sh2;
    ...
    uint recordId = sensorToRecordMap[i].recordId;
    ...
    // Read an FRS record
    uint status = opProcess(pSh2, &getFrsOp);
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getFrs(uint recordId, uint* pData, uint* words)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setFrs(uint recordId, uint* pData, uint words)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getErrors(int severity, sh2_ErrorRecord_t* pErrors, uint* numErrors)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getCounts(sh2_SensorId_t sensorId, sh2_Counts_t* pCounts)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_clearCounts(sh2_SensorId_t sensorId)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setTareNow(int axes,    // SH2_TARE_X | SH2_TARE_Y | SH2_TARE_Z
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setReorientation(sh2_Quaternion_t* orientation)
{
    sh2_t* pSh2 = &_sh2;
    ...
    int* p = pSh2->opData.sendCmd.req.p;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getOscType(sh2_OscType_t* pOscType)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setCalConfig(int sensors)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_getCalConfig(int* pSensors)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setDcdAutoSave(bool enabled)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_flush(sh2_SensorId_t sensorId)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_clearDcdAndReset(void)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_startCal(uint interval_us)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_finishCal(sh2_CalStatus_t* status)
{
    sh2_t* pSh2 = &_sh2;
 ...
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
uint sh2_setIZro(sh2_IZroMotionIntent_t intent)
{
    sh2_t* pSh2 = &_sh2;
